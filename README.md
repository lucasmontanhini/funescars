...
> 🏛️ **Instituição:** Faculdade de Tecnologia do Estado de São Paulo (FATEC)
...
> 🛠️ *O script completo de criação das tabelas está disponível neste repositório sob o nome `schema.sql`.*
...
* **Garantia de Tipagem:** Dados armazenados estritamente com seus tipos correspondentes (`VARCHAR`, `INT`, `DATE`, `NUMERIC`), garantindo a integridade referencial.
...
### ➕ CREATE (Inserção)
Para popular inicialmente o banco de dados, foi utilizado um script automatizado (`popular.sql`). No entanto, as inserções manuais via comandos `INSERT INTO` também foram validadas com sucesso:
